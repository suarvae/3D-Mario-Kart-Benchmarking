...
public class ItemManager : MonoBehaviour
{
    ...
    IEnumerator UseStar()
    {
      //  ApplyEffect("Star", 7.5f);
        ...
        Debug.Log("using star");
        item_gameobjects[item_index].SetActive(false);
        Debug.Log("about to turn on star power ip");
        StarPowerUp = true;
        Debug.Log("STAR UP turned to true");
        for(int i = 0; i < playerRenderers.Length; i++)
        {
            playerRenderers[i].material = starMat;
            playerRenderers[i].sharedMaterial = starMat;
        }
        Debug.Log("material is on");
        ...
        Debug.Log("should be done using star");
    ...
    IEnumerator UseBullet()
    {
        ...
        // Initialize bullet path
    //    bulletPlayer.GetComponent<BulletEffect>().InitializePath(path);
        ...
        Debug.Log("done using bullet");
    }
    void ActivateBulletEffect()
    {
        if (isBullet)
        {
            TogglePlayerModel(false); // Deactivate player model
                                      // Align bullet to path center using the new method
            Vector3 pathCenter = path.GetComponent<DriverPath>().GetPathCenter();
            bulletPlayer.transform.position = pathCenter;
            bulletPlayer.SetActive(true);
        }
    }
    void TogglePlayerModel(bool isActive)
    {
