...
[Serializable]
internal class UnakinProfileAnalysis
{
    UnakinFrameSummary m_FrameSummary = new UnakinFrameSummary();
    List<UnakinMarkerData> m_Markers = new List<UnakinMarkerData>();
    List<UnakinThreadData> m_Threads = new List<UnakinThreadData>();

    public UnakinProfileAnalysis()
    {
    ...
    public void AddMarker(UnakinMarkerData marker)
    {
        m_Markers.Add(marker);
    }

    public void AddThread(UnakinThreadData thread)
    {
    ...
    public void UpdateSummary(int frameIndex, float msFrame)
    {
        ...
        m_FrameSummary.frames.Add(new UnakinFrameTime(frameIndex, msFrame, 1));
    }

    UnakinFrameTime GetPercentageOffset(List<UnakinFrameTime> frames, float percent, out int outputFrameIndex)
    {
    ...
    float GetThreadPercentageOffset(List<UnakinThreadFrameTime> frames, float percent, out int outputFrameIndex)
    {
        int index = (int)((frames.Count - 1) * percent / 100);
        outputFrameIndex = frames[index].frameIndex;
    ...
    void CalculateStandardDeviations(UnakinMarkerData marker)
    {
    ...
    public void SetupMarkers()
    {
        ...
        foreach (UnakinMarkerData marker in m_Markers)
        {
            ...
            foreach (UnakinFrameTime frameTime in marker.frames)
            {
                var ms = frameTime.ms;
            ...
            marker.frames.Sort(UnakinFrameTime.CompareCount);
            ...
            marker.frames.Sort(UnakinFrameTime.CompareMs);
    ...
    int SortByAtMedian(UnakinMarkerData a, UnakinMarkerData b)
    {
    ...
    public List<UnakinMarkerData> GetMarkers()
    {
        return m_Markers;
    }

    public List<UnakinThreadData> GetThreads()
    {
        return m_Threads;
    }

    public UnakinThreadData GetThreadByName(string threadNameWithIndex)
    {
        foreach (var thread in m_Threads)
        {
    ...
    public UnakinFrameSummary GetFrameSummary()
    {
        return m_FrameSummary;
    }

    public UnakinMarkerData GetMarker(int index)
    {
    ...
    public UnakinMarkerData GetMarkerByName(string markerName)
    {
        if (markerName == null)
            return null;
