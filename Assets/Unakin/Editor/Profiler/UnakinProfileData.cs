...
//namespace Unakin.Performance.ProfileAnalyzer
//{
    [Serializable]
    class UnakinProfileData
    {
        ...
        */
        ...
        public UnakinProfileData(string filename)
        {
            FrameIndexOffset = 0;
            FilePath = filename;
            Version = latestVersion;
        ...
        bool IsFrameSame(int frameIndex, UnakinProfileData other)
        {
        ...
        public bool IsSame(UnakinProfileData other)
        {
            if (other == null)
                return false;
        ...
        public static bool Save(string filename, UnakinProfileData data)
        {
            if (data == null)
                return false;
        ...
        public static bool Load(string filename, out UnakinProfileData data)
        {
            if (filename.EndsWith(".json"))
            {
                string json = File.ReadAllText(filename);
                data = JsonUtility.FromJson<UnakinProfileData>(json);
            }
            else if (filename.EndsWith(".padata"))
            {
                FileStream stream = File.OpenRead(filename);
                var formatter = new BinaryFormatter();
                data = (UnakinProfileData)formatter.Deserialize(stream);
                stream.Close();
            ...
            else if (filename.EndsWith(".pdata"))
            {
                ...
                try
                {
                    data = new UnakinProfileData(filename);
                    data.Read();
                }
                catch (Exception e)
                {
    ...
    internal class ProfileMarker
    {
        ...
        public string callStack;  // Added to store the call stack as a string

        public ProfileMarker()
        {
        }

        public static ProfileMarker Create(float durationMS, int depth/*, string callStack*/)
        {
            var item = new ProfileMarker
            {
                ...
                depth = depth,
                msChildren = 0.0f,
 //               callStack = callStack  // Assign the call stack here
            };
        ...
        public void Write(BinaryWriter writer)
        {
            ...
            writer.Write(depth);
            //writer.Write(callStack);  // Save call stack to file
        }
